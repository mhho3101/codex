...
```bash
node C:/Users/Admin/.agents/skills/claude-vision-skill/vision.js "<absolute image path>" "<prompt>"
```

For an image URL:

```bash
node C:/Users/Admin/.agents/skills/claude-vision-skill/vision.js --url "<image url>" "<prompt>"
```

When the user pastes an image into the chat but no file path or URL is visible:

```bash
node C:/Users/Admin/.agents/skills/claude-vision-skill/vision.js --clipboard "<prompt>"
```
